...
class  PedidoItem(Base):
    ...
    quantidade_id = Column(Integer,nullable=False)

    pedido = relationship("Pedido", back_populates= 'itens')
    produto = relationship("Produto", back_populates= 'itens')

    def __repr__(self):
        return f"<PedidoItem(nome_produto='{self.produto.nome}'),\
            quantidade ={self.quantidade})>"
    

engine = create_engine(
    'mysql+pymysql://root:alunolab@localhost:3303/loja_virtual',echo=True, future=True)

Session = sessionmaker(bind=engine, future=True)

with Session() as session:
    #insert
    anderson = Cliente(nome ='Anderson',email='andinho@senai.br')
    elon_musk = Cliente(nome = 'Elon Musk',email= 'cocquinha_zero@senai.br')
    ...
    session.commit()
        ...
        # criar pedidos
    pedido1 = Pedido(cliente=elon_musk, data=date.today())
    item1 = PedidoItem(pedido=pedido1, produto= p1, quantidade=5)
    item2 = PedidoItem(pedido=pedido1, produto= p3, quantidade=4)
...
with Session() as sesssion:
    ...
    for pedido in elon.pedidos:
        ...
        for item  in pedido.itens:
            print(f'{item.produto.nome}, {item.produto.preco}, {item.quantidade}')
